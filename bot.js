...
- Штрихкод должен быть только из цифр, без пробелов и символов. Допустимая длина: от 8 до 13 цифр.
...
function formatCatalogResponse(row) {
  const parts = [
    ...
    row[12],
    row[9],
  ].map((value) => String(value || 'не указано').trim() || 'не указано');
...
5. barcode
- Только цифры, от 8 до 13 символов, без пробелов и знаков.
